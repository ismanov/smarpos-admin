...
export const fetchServicesItems = createEffect<string, any[], AxiosError>({
  handler: Repository.service.fetchServicesItems
});
...
export const fetchAgreementsList = createEffect<FetchAgreementsListParamsI, AgreementListItemI[], AxiosError>().use(async params => {
  ...
  return await Repository.service.fetchAgreementsList(params, agreementsListCancelToken.token);
});
