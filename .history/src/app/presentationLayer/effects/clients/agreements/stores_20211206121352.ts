...
import { resetClientCard } from "../events";
...
export const $tariffsItems = createStore<TariffsItemsStoreI>({ loading: false, data: [], error: undefined })
  ...
  export const $serviceItems = createStore<TariffsItemsStoreI>({ loading: false, data: [], error: undefined })
  .on(effects.fetchServicesItems.pending, (state, pending) => ({
    ...state,
    loading: pending
  }))
  .on(effects.fetchServicesItems.done, (state, response) => ({
    ...state,
    data: response.result,
    error: undefined
  }))
  .on(effects.fetchServicesItems.fail, (state, response) => ({
    ...state,
    data: [],
    error: response.error.response && response.error.response.data
  }));
